...
// Azimuthal exit angle for lobe p at impact h.
// Uses modified IOR etap = sqrt(eta^2 - sin^2(theta_d)) / cos(theta_d).
...
// Evaluate N_p(phi) via 16-point midpoint quadrature.
// Returns RGB attenuation-weighted azimuthal scattering.
static vec3 eval_Np(int p, float phi, float etap, float cos_theta_d,
                    const vec3& sa, float beta_n)
{
    const int N = 16;
    vec3 acc(0.f);
    for (int j = 0; j < N; ++j) {
        ...
        // Absorption over chord length through fiber
        vec3  T = {
            expf(-2.f*sa.x*cos_gi / cos_theta_d),
    ...
    return acc * (1.f/N);   // midpoint: * (2/N)/2 = 1/N
}
               ...
               const DeonParams& p)
{
    ...
    if (cos2_td < 1e-7f) return vec3(0.f);

    // --- azimuthal angle (unsigned, 0..pi) ---
    ...
    // --- per-lobe contribution ---
    const float alpha[3] = { -p.alpha, 0.5f*p.alpha, -1.5f*p.alpha };
    const float beta_p[3] = { p.beta_R, p.beta_TT, p.beta_TRT };

    vec3 result(0.f);
    for (int lobe = 0; lobe < 3; ++lobe) {
        // 0.5 corrects the Jacobian: M is normalized in theta_h but the
        // rendering integral runs over theta_o = 2*theta_h → factor of 2.
                 ...
                 vec3& wi_out, float& pdf_out)
{
    ...
    // --- approximate lobe weights via A_p energy at h=0, theta_d=0 ---
    float F0 = fresnel(p.eta, 1.f);
    float w[3] = { F0, (1.f-F0)*(1.f-F0), (1.f-F0)*(1.f-F0)*F0 };
    float wsum = w[0]+w[1]+w[2];
    if (wsum < 1e-7f) { pdf_out=0.f; return vec3(0.f); }
    for (float& x : w) x /= wsum;

    // --- select lobe ---
    int lobe = 2;
    float cdf = 0.f;
    for (int i = 0; i < 3; ++i) { cdf += w[i]; if (u1 < cdf){lobe=i; break;} }

    const float alpha_p[3] = { -p.alpha, 0.5f*p.alpha, -1.5f*p.alpha };
    const float beta_p[3]  = { p.beta_R, p.beta_TT, p.beta_TRT };

    // --- sample theta_h via Box-Muller, derive theta_i ---
    u2 = std::max(u2, 1e-7f);
    ...
    float td      = (to - ti)*0.5f;
    float cos_td  = cosf(td);
    if (cos_td < 1e-5f) { pdf_out=0.f; return vec3(0.f); }

    float etap = sqrtf(std::max(0.f, p.eta*p.eta - (1.f-cos_td*cos_td))) / cos_td;
    etap = std::max(etap, 1.001f);

    // --- sample phi from 64-point N_p CDF ---
    const int NPHI = 64;
    float table[NPHI];
    float tsum = 0.f;
    ...
    if (tsum < 1e-7f) {
        ...
        for (int k = 0; k < NPHI; ++k) {
            acc += table[k];
            if (acc >= target) { phi_s = (k+0.5f)*(2.f*PI/NPHI); break; }
        }
    }

    // --- build wi in (T, phi_x, phi_y) frame ---
    vec3 wo_p = wo - so*T;
    ...
    float Np_val = (tsum > 1e-7f)
        ? lum(eval_Np(lobe, phi_s, etap, cos_td, p.sigma_a, p.beta_n)) / (tsum*(2.f*PI/NPHI))
        : 1.f/(2.f*PI);

    pdf_out = w[lobe] * M_pdf * 0.5f * Np_val;
    if (pdf_out < 1e-7f) { pdf_out=0.f; return vec3(0.f); }
